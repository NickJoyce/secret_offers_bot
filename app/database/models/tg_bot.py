...
class ChannelPost(Base):
    ...
    is_buttons_deleted: Mapped[bool] = mapped_column(Boolean, default=False, comment="Флаг удаления кнопок")
...
class BlackList(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    name: Mapped[str] = mapped_column(String(255), default=None, nullable=True, comment="Призвольное имя")
...
class DeeplinkRequest(Base):
    ...
    tg_id: Mapped[int] = mapped_column(BigInteger, comment="ID пользователя в Telegram")
    is_registred: Mapped[bool] = mapped_column(Boolean, default=True, comment="Зарегистрирован?")
